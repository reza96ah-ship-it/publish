...
describe('Storage — magic byte validation', () => {
  ...
  it('validates GIF magic bytes', () => {
    const buffer = Buffer.from([
      ...
      0x90, 0x01, // width = 400 (little-endian)
      0x90, 0x01, // height = 400
      0x00, 0x00, // padding to reach 12 bytes
    ])
    const result = validateMagicBytes(buffer)
    expect(result.valid).toBe(true)
  ...
  it('rejects invalid file (exe renamed to jpg)', () => {
    const buffer = Buffer.from([0x4d, 0x5a, 0x90, 0x00, 0x03, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00])
    const result = validateMagicBytes(buffer)
