...
export function announce(message: string, politeness: "polite" | "assertive" = "polite", clearAfter = 1500) {
...
export function useAriaLive() {
  const announceChange = useCallback((count: number, label: string, politeness: "polite" | "assertive" = "polite") => {
    if (count === 0) {
...
export function useAnnounceValue(value: number, label: string, politeness: "polite" | "assertive" = "polite") {
  ...
  useEffect(() => {
    if (prevRef.current !== value) {
      ...
      announce(`${direction} ${absDiff.toLocaleString("fa-IR")} ${label}، مجموع ${value.toLocaleString("fa-IR")}`, politeness);
