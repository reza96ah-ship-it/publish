...
export async function POST(req: NextRequest) {
  try {
    ...
    try { workspace = await getWorkspace(); } catch {}
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        const generators = validPlatforms.map(async (p) => {
          try {
            controller.enqueue(encoder.encode(`data: ${JSON.stringify({ status: "thinking", platform: p })}\n\n`));
            let first = true;
            for await (const chunk of streamCaption(
              topic, p as Platform, workspace ?? undefined,
              tone as Tone | undefined, role as CreatorRole | undefined,
              goal as ContentGoal | undefined, length as CaptionLength | undefined,
            )) {
              if (first) {
                controller.enqueue(encoder.encode(`data: ${JSON.stringify({ status: "streaming", platform: p })}\n\n`));
                first = false;
              }
              controller.enqueue(encoder.encode(`data: ${JSON.stringify({ content: chunk, platform: p })}\n\n`));
            }
