/**
 * Issue #255: Public API v1 — list inbox messages.
 *
 *   GET /api/v1/inbox  — cursor-paginated inbox for the token's workspace.
 *   Unread messages sort first, then by id desc. Requires scope `inbox:read`.
 */
...
export async function GET(req: NextRequest) {
  const guard = await requireApiToken(req, ['inbox:read'])
  ...
  const result = await publicApiService.listInbox(
