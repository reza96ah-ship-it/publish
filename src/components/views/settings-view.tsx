...
import { SectionTitle, Skeleton, LoadingState, AnimatedTabs, EmptyState } from "@/components/dashboard/shared";
...
const PLAN_TIERS = [
  ...
  },
  {
    ...
    features: ["۵ پلتفرم", "نامحدود پست", "۵ کاربر", "تحلیل‌های پیشرفته", "اتوماسیون کامنت به DM", "پشتیبانی اولویت‌دار"],
    current: false,
    popular: true,
  },
  {
    ...
    features: ["پلتفرم‌های نامحدود", "۱۰ کاربر", "نقش‌های سفارشی", "برند چندگانه", "گزارش‌های سفارشی", "API دسترسی"],
    current: false,
  },
  {
    id: "enterprise",
    ...
    features: ["بدون محدودیت", "کاربران نامحدود", "SSO و SAML", "SLA 99.9٪", "پشتیبانی اختصاصی", "آموزش تیمی"],
    current: false,
  },
];

const NOTIFICATION_TOGGLES = [
  { id: "publish_success", label: "موفقیت انتشار", desc: "هنگام انتشار موفق پست", icon: Check, defaultOn: true },
  { id: "publish_failed", label: "شکست انتشار", desc: "هنگام ناموفق بودن انتشار", icon: Bell, defaultOn: true },
  { id: "approval_requested", label: "درخواست تأیید", desc: "هنگام نیاز به تأیید محتوا", icon: Users, defaultOn: true },
  { id: "inbox_new", label: "پیام جدید", desc: "هنگام دریافت کامنت یا پیام مستقیم", icon: Bell, defaultOn: false },
  { id: "token_expiring", label: "انقضای توکن", desc: "هشدار قبل از انقضای توکن پلتفرم", icon: Bell, defaultOn: true },
  { id: "channel_disconnected", label: "قطع اتصال کانال", desc: "هنگام قطع شدن پلتفرم", icon: Bell, defaultOn: true },
];

export function SettingsView() {
  const [tab, setTab] = useState<"overview" | "brand" | "team" | "billing" | "notifications">("overview");
...
function OverviewForm({ ws }: { ws: Workspace }) {
  ...
  const set = (key: keyof Workspace, value: string) =>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select value={form.timezone ?? "Asia/Tehran"} onValueChange={(v) => set("timezone", v)}>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div className="lg:col-span-2 n-card p-5">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <Label className="text-[12px] text-ink-secondary mb-1.5 block">دستورالعمل‌های محتوایی</Label>
              <Textarea
                dir="rtl"
                rows={3}
      ...
      <div className="n-card p-5 h-fit sticky top-4">
        ...
        <div className="n-card-compact p-4">
          ...
          <div className="aspect-square w-full rounded-xl mb-3" style={{ background: `${form.brandAccentColor ?? "#2563EB"}22` }} />
    ...
    <div className="n-card p-0 overflow-hidden">
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border hover:bg-transparent">
                  <TableHead className="text-right text-[11px] text-ink-tertiary font-[700]">عضو</TableHead>
                  <TableHead className="text-right text-[11px] text-ink-tertiary font-[700] hidden sm:table-cell">ایمیل</TableHead>
                  <TableHead className="text-right text-[11px] text-ink-tertiary font-[700]">نقش</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {(members ?? []).map((m) => (
                    ...
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <Avatar className="size-9">
                          ...
                          <AvatarFallback className="text-[12px]">{m.name.slice(0, 1)}</AvatarFallback>
                        </Avatar>
                        <span className="text-[13px] font-[600] text-ink-primary">{m.name}</span>
                      </div>
                    </TableCell>
                    <TableCell className="hidden sm:table-cell text-[12px] text-ink-secondary" dir="ltr">
                      {m.email}
                    </TableCell>
                    <TableCell>
                      <span className={cn("text-[10px] font-[700] px-2 py-0.5 rounded-full border", ROLE_COLOR[m.role] ?? ROLE_COLOR.viewer)}>
                        {m.roleLabel}
                      </span>
                    </TableCell>
                  </TableRow>
      ...
      <Dialog open={inviteOpen} onOpenChange={setInviteOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setInviteOpen(false)}>انصراف</Button>
            <Button
              onClick={() => {
                if (!email) {
    ...
    <div className="space-y-4">
      <div className="n-card p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-lg font-[700] text-ink-primary">{planLabel[currentPlan] ?? currentPlan}</p>
          ...
          {PLAN_TIERS.map((tier) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-[18px] font-[700] text-ink-primary mt-1">
                  {tier.priceLabel}
                </p>
      ...
      <div className="n-card p-0 overflow-hidden">
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-border hover:bg-transparent">
              <TableHead className="text-right text-[11px] text-ink-tertiary font-[700]">شماره فاکتور</TableHead>
              <TableHead className="text-right text-[11px] text-ink-tertiary font-[700]">تاریخ</TableHead>
              <TableHead className="text-right text-[11px] text-ink-tertiary font-[700]">مبلغ</TableHead>
              <TableHead className="text-right text-[11px] text-ink-tertiary font-[700]">وضعیت</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            <TableRow className="border-border">
              <TableCell className="text-[12px] font-[600] text-ink-primary" dir="ltr">INV-1403-003</TableCell>
              <TableCell className="text-[12px] text-ink-secondary">{formatJalali(new Date(Date.now() - 1000 * 60 * 60 * 24 * 30))}</TableCell>
              <TableCell className="text-[12px] text-ink-primary num-tabular">{toPersianDigits("۲۹۰,۰۰۰")} تومان</TableCell>
              <TableCell>
                <Badge className="text-[10px] bg-success-soft text-success border-success/20" variant="outline">پرداخت‌شده</Badge>
              </TableCell>
            </TableRow>
            <TableRow className="border-border">
              <TableCell className="text-[12px] font-[600] text-ink-primary" dir="ltr">INV-1403-002</TableCell>
              <TableCell className="text-[12px] text-ink-secondary">{formatJalali(new Date(Date.now() - 1000 * 60 * 60 * 24 * 60))}</TableCell>
              <TableCell className="text-[12px] text-ink-primary num-tabular">{toPersianDigits("۲۹۰,۰۰۰")} تومان</TableCell>
              <TableCell>
                <Badge className="text-[10px] bg-success-soft text-success border-success/20" variant="outline">پرداخت‌شده</Badge>
              </TableCell>
            ...
            <TableRow className="border-border">
              <TableCell className="text-[12px] font-[600] text-ink-primary" dir="ltr">INV-1403-001</TableCell>
              <TableCell className="text-[12px] text-ink-secondary">{formatJalali(new Date(Date.now() - 1000 * 60 * 60 * 24 * 90))}</TableCell>
              <TableCell className="text-[12px] text-ink-primary num-tabular">{toPersianDigits("۲۹۰,۰۰۰")} تومان</TableCell>
              <TableCell>
                <Badge className="text-[10px] bg-success-soft text-success border-success/20" variant="outline">پرداخت‌شده</Badge>
              </TableCell>
