  ...
  return (
    <div className="n-card p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div className="leading-tight">
            ...
            <p className="text-[11px] text-ink-tertiary mt-0.5 leading-tight">وضعیت لحظه‌ای سیستم</p>
          </div>
        </div>
        {data && (
          <span className={`inline-flex items-center gap-1 text-[10.5px] font-[600] px-2 py-1 rounded-md border ${healthTone[data.health] ?? healthTone.warning}`}>
            {(() => {
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-2">
        <Stat icon={CheckCircle2} label="منتشرشده امروز" value={data?.publishedToday} color="text-success" bg="bg-success-soft" />
        <Stat icon={Clock} label="در صف" value={data?.queued} color="text-info" bg="bg-info-soft" />
        <Stat icon={Activity} label="در حال پردازش" value={data?.processing} color="text-accent" bg="bg-accent-soft" />
        <Stat icon={AlertTriangle} label="ناموفق" value={data?.failed} color="text-danger" bg="bg-danger-soft" />
        <Stat icon={CheckCircle2} label="در انتظار تأیید" value={data?.pendingApproval} color="text-warning" bg="bg-warning-soft" />
        <Stat icon={Inbox} label="خوانده‌نشده" value={data?.unreadInbox} color="text-info" bg="bg-info-soft" />
        <Stat icon={Flag} label="کمپین‌های فعال" value={data?.activeCampaigns} color="text-accent" bg="bg-accent-soft" />
      </div>
    </div>
  );
}

function Stat({ icon: Icon, label, value, color, bg }: { icon: typeof Activity; label: string; value?: number; color: string; bg: string }) {
  return (
    <div className="n-card-compact flex flex-col gap-1.5 p-2.5">
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[10px] font-[500] text-ink-secondary leading-tight line-clamp-1">{label}</span>
      </div>
