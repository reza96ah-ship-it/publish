/**
 * Realtime JWT verification + signing (Issue #151, ASVS L2 V3.5.2/V9.2.1).
 *
 * This module is shared between:
 *   - The Next.js app (`src/app/api/realtime-token/route.ts`) — ISSUES realtime
 *     JWTs for socket.io handshake auth.
 *   - The realtime mini-service (`mini-services/realtime/lib/jwt.ts`) — VERIFIES
 *     realtime JWTs on every socket.io connection.
 *
 * Sharing the implementation guarantees the issuer and verifier use the same
 * claim profile, algorithm pinning, and clock-skew tolerance — without this,
 * the realtime service would silently reject every token issued by the app.
 *
 * Token profile (all 9 claims required):
 ...
 * Sign a realtime JWT.
 *
 * Used by the app's `/api/realtime-token` issuer endpoint. Sharing this
 * function with the verifier guarantees the profile matches exactly.
 */
